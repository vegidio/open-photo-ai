...
// bit-identical to the exposure checkpoint and none at all to lol_v1. The 14 unmatched are the conv_large
// BatchNorms, folded into their convolutions at export. That also agrees with the upstream img_demo.py, where
// `--task exposure` loads exactly that file.
//
...
// build outright ("axis 4 is not in valid range [-4,3]"). The tell was that the shipping graph landed on the same
// number through CoreML as through the CPU provider in both precisions, around 640ms at 1024x688.
//
// No provider setting reaches that, so the fixed shape is not a tuning choice but the only way onto the GPU. At a
// fixed 1024x1024 the folded graph is a single CoreML partition (fp32; fp16 adds only the trailing fp32 output Cast,
// as lyon's does) and the same sweep gives about 10ms in both precisions - a 98% cut, and understated, since the
...
// over 27 crops above the old 1024 ceiling, 1024 holds 41.9 dB median against what paris renders today where 768
// manages 31.2 dB and 512 27.3 dB. 1024 wins because it is the size those images were already being resized to; only
// the padding is new. Images BELOW the old ceiling change more, and that is the real cost of this: they used to run
// at their native size and now get resampled to the canvas like everything else.
