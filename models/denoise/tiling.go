...
// denoiseTile runs the ML model inference and removes padding from the result
func denoiseTile(session *ort.DynamicAdvancedSession, tile image.Image, tileW, tileH int) (image.Image, error) {
	// Run inference
	denoisedTile, err := runInference(session, tile)
	if err != nil {
		return nil, errors.Wrap(err, "failed to run inference")
	}

	// Remove padding by cropping to the actual content size (denoise does not change dimensions)
	croppedTile := imaging.Crop(denoisedTile, image.Rect(0, 0, tileW, tileH))
