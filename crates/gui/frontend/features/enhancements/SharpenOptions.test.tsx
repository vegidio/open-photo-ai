...
/** Puts one sharpen on the current image, draws the list, and opens its options. */
const open = async (operation = sharpen("moscow", "fp32", 1)) => {
    ...
    await screen.findByText(/Moscow|Petersburg|Novgorod/);
...
describe("a sharpen's options", () => {
    it("offer the three models and the strength, labelled Strength, as a percentage over the published range", async () => {
        await open(sharpen("moscow", "fp32", 1.25));

        expect(screen.getByRole("radio", { name: "Moscow HD" })).toBeChecked();
        expect(screen.getByRole("radio", { name: "Petersburg HD" })).toBeInTheDocument();
        expect(screen.getByRole("radio", { name: "Novgorod HD" })).toBeInTheDocument();
    ...
    it("write a typed percentage as the library's unit strength", async () => {
        ...
        await waitFor(() => expect(stack()).toEqual([sharpen("moscow", "fp32", 1.5)]));
    ...
    it("bring a typed strength beyond the range down to the largest the model publishes", async () => {
        ...
        await waitFor(() => expect(stack()).toEqual([sharpen("moscow", "fp32", 3)]));
    });
    ...
    it("bring a typed negative strength up to none", async () => {
        ...
        await waitFor(() => expect(stack()).toEqual([sharpen("moscow", "fp32", 0)]));
    });
    ...
    it("write a dragged strength once, when the slider is released", async () => {
        ...
        expect(stack()).toEqual([sharpen("moscow", "fp32", 2.25)]);
    });

    it("keep the strength when the model changes", async () => {
        await open(sharpen("moscow", "fp32", 1.5));

        fireEvent.click(screen.getByRole("radio", { name: "Novgorod HD" }));

        await waitFor(() => expect(stack()).toEqual([sharpen("novgorod", "fp32", 1.5)]));
    });
});
