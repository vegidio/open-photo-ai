...
// Variant is the complete description of one denoise model. Everything that distinguishes stockholm from malmo lives
...
type Variant struct {
	...
	// variant's findings to another because both denoise is how a profile ends up pessimising a model it was never
	// measured against.
...
// Op identifies a denoise run. The variant and precision form the model identity; the intensity is deliberately not
...
func (o Op) Id() string {
	return fmt.Sprintf("dn_%s_%s", o.variant.Codename, o.precision)
}
...
// Model is a loaded denoise session together with the variant it was built from.
type Model struct {
	name      string
	operation Op
	variant   *Variant
