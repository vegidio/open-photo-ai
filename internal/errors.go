...
//
// It's what makes the CPU fallback in GetOrCreateModel possible: a session that can't be built is usually the
...
//
// It lives here rather than next to CreateSession in internal/utils so that GetOrCreateModel can test for it without
